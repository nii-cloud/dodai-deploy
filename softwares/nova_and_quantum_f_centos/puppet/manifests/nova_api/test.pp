class nova_and_quantum_centos_f::nova_api::test {
    ...
    file {
        "/var/lib/nova/test.sh":
            alias => "test.sh",
            content => template("nova_and_quantum_centos_f/test.sh.erb");

        "/var/lib/nova/$image_file_name":
            alias => "$image_file_name",
            source => "puppet:///modules/nova_and_quantum_centos_f/$image_file_name";
    }
